{
  "nbformat": 4,
  "nbformat_minor": 0,
  "metadata": {
    "colab": {
      "provenance": [],
      "toc_visible": true,
      "authorship_tag": "ABX9TyMKmNvDRwoAXoyr6FKwfvGC",
      "include_colab_link": true
    },
    "kernelspec": {
      "name": "python3",
      "display_name": "Python 3"
    },
    "language_info": {
      "name": "python"
    }
  },
  "cells": [
    {
      "cell_type": "markdown",
      "metadata": {
        "id": "view-in-github",
        "colab_type": "text"
      },
      "source": [
        "<a href=\"https://colab.research.google.com/github/Muhozgu/data_pipeline_csv_to_database/blob/main/eda_pyspark.py\" target=\"_parent\"><img src=\"https://colab.research.google.com/assets/colab-badge.svg\" alt=\"Open In Colab\"/></a>"
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "!sudo apt update"
      ],
      "metadata": {
        "id": "sYZgjrX_xefN",
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "outputId": "786a2eb6-ed6c-4f76-ed69-2d99f30d5efd"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "\u001b[33m\r0% [Working]\u001b[0m\r            \rHit:1 https://cloud.r-project.org/bin/linux/ubuntu jammy-cran40/ InRelease\n",
            "Hit:2 https://cli.github.com/packages stable InRelease\n",
            "Hit:3 http://archive.ubuntu.com/ubuntu jammy InRelease\n",
            "Hit:4 http://security.ubuntu.com/ubuntu jammy-security InRelease\n",
            "Hit:5 https://r2u.stat.illinois.edu/ubuntu jammy InRelease\n",
            "Hit:6 http://archive.ubuntu.com/ubuntu jammy-updates InRelease\n",
            "Hit:7 http://archive.ubuntu.com/ubuntu jammy-backports InRelease\n",
            "Hit:8 https://ppa.launchpadcontent.net/deadsnakes/ppa/ubuntu jammy InRelease\n",
            "Hit:9 https://ppa.launchpadcontent.net/ubuntugis/ppa/ubuntu jammy InRelease\n",
            "Reading package lists... Done\n",
            "Building dependency tree... Done\n",
            "Reading state information... Done\n",
            "63 packages can be upgraded. Run 'apt list --upgradable' to see them.\n",
            "\u001b[1;33mW: \u001b[0mSkipping acquire of configured file 'main/source/Sources' as repository 'https://r2u.stat.illinois.edu/ubuntu jammy InRelease' does not seem to provide it (sources.list entry misspelt?)\u001b[0m\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "!apt-get install openjdk-8-jdk-headless -qq > /dev/null\n",
        "!wget -q https://dlcdn.apache.org/spark/spark-4.1.1/spark-4.1.1-bin-hadoop3.tgz"
      ],
      "metadata": {
        "id": "tn-ut1ZkrMPA"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "!tar xf spark-4.1.1-bin-hadoop3.tgz"
      ],
      "metadata": {
        "id": "57y19uhVrnUP"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "!pip install -q findspark"
      ],
      "metadata": {
        "id": "q8nBbk2ksN4l"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "!pip install pyspark"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "r4jHt740sRML",
        "outputId": "e1aea7cb-741d-4140-8c26-4233c22fde4f"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "Requirement already satisfied: pyspark in /usr/local/lib/python3.12/dist-packages (4.0.2)\n",
            "Requirement already satisfied: py4j<0.10.9.10,>=0.10.9.7 in /usr/local/lib/python3.12/dist-packages (from pyspark) (0.10.9.9)\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "!pip install py4j"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "q7gkAaU_sUlF",
        "outputId": "03c9a465-5a2d-45fe-e74e-336133f1f0cf"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "Requirement already satisfied: py4j in /usr/local/lib/python3.12/dist-packages (0.10.9.9)\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "import os\n",
        "import sys\n",
        "os.environ[\"JAVA_HOME\"] = \"/usr/lib/jvm/java-8-openjdk-amd64\"\n",
        "os.environ[\"SPARK_HOME\"] = \"/content/spark-4.1.1-bin-hadoop3\"\n",
        "\n",
        "\n",
        "import findspark\n",
        "findspark.init()\n",
        "findspark.find()\n",
        "\n",
        "import pyspark\n",
        "\n",
        "from pyspark.sql import DataFrame, SparkSession\n",
        "from typing import List\n",
        "import pyspark.sql.types as T\n",
        "import pyspark.sql.functions as F\n",
        "\n",
        "spark= SparkSession \\\n",
        "       .builder \\\n",
        "       .appName(\"Our First Spark Example\") \\\n",
        "       .getOrCreate()\n",
        "\n",
        "spark"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/",
          "height": 219
        },
        "collapsed": true,
        "id": "uspujanisXO5",
        "outputId": "b3664ab7-5b0c-45cf-89c0-596ffade4eb4"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "execute_result",
          "data": {
            "text/plain": [
              "<pyspark.sql.session.SparkSession at 0x781d85b94b60>"
            ],
            "text/html": [
              "\n",
              "            <div>\n",
              "                <p><b>SparkSession - in-memory</b></p>\n",
              "                \n",
              "        <div>\n",
              "            <p><b>SparkContext</b></p>\n",
              "\n",
              "            <p><a href=\"http://54b2eac3db6a:4040\">Spark UI</a></p>\n",
              "\n",
              "            <dl>\n",
              "              <dt>Version</dt>\n",
              "                <dd><code>v4.0.2</code></dd>\n",
              "              <dt>Master</dt>\n",
              "                <dd><code>local[*]</code></dd>\n",
              "              <dt>AppName</dt>\n",
              "                <dd><code>Our First Spark Example</code></dd>\n",
              "            </dl>\n",
              "        </div>\n",
              "        \n",
              "            </div>\n",
              "        "
            ]
          },
          "metadata": {},
          "execution_count": 43
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "df = spark.read.csv(\n",
        "    \"/content/bmw_data.csv\",\n",
        "    header=True,\n",
        "    inferSchema=True)\n",
        "\n",
        "df.show()\n",
        "df.printSchema()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "j1Da59nzs2Kc",
        "outputId": "9913e7a2-3cf4-4b96-8525-b58cd104d860"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|   Model|Year|       Region| Color|Fuel_Type|Transmission|Engine_Size_L|Mileage_KM|Price_USD|Sales_Volume|Sales_Classification|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|5 Series|2016|         Asia|   Red|   Petrol|      Manual|          3.5|    151748|    98740|        8300|                High|\n",
            "|      i8|2013|North America|   Red|   Hybrid|   Automatic|          1.6|    121671|    79219|        3428|                 Low|\n",
            "|5 Series|2022|North America|  Blue|   Petrol|   Automatic|          4.5|     10991|   113265|        6994|                 Low|\n",
            "|      X3|2024|  Middle East|  Blue|   Petrol|   Automatic|          1.7|     27255|    60971|        4047|                 Low|\n",
            "|7 Series|2020|South America| Black|   Diesel|      Manual|          2.1|    122131|    49898|        3080|                 Low|\n",
            "|5 Series|2017|  Middle East|Silver|   Diesel|      Manual|          1.9|    171362|    42926|        1232|                 Low|\n",
            "|      i8|2022|       Europe| White|   Diesel|      Manual|          1.8|    196741|    55064|        7949|                High|\n",
            "|      M5|2014|         Asia| Black|   Diesel|   Automatic|          1.6|    121156|   102778|         632|                 Low|\n",
            "|      X3|2016|South America| White|   Diesel|   Automatic|          1.7|     48073|   116482|        8944|                High|\n",
            "|      i8|2019|       Europe| White| Electric|      Manual|          3.0|     35700|    96257|        4411|                 Low|\n",
            "|3 Series|2012|  Middle East| White| Electric|      Manual|          2.4|    101595|   117995|         500|                 Low|\n",
            "|      i8|2016|North America|  Blue|   Hybrid|      Manual|          1.9|    116863|    74539|        8252|                High|\n",
            "|7 Series|2020|North America|Silver|   Diesel|   Automatic|          3.8|     27403|   100015|        8111|                High|\n",
            "|5 Series|2020|       Africa| White| Electric|      Manual|          2.3|    163444|   119486|        4668|                 Low|\n",
            "|      X1|2017|         Asia| Black| Electric|   Automatic|          1.6|    107918|    34078|        7291|                High|\n",
            "|      M3|2014|North America|Silver|   Hybrid|      Manual|          3.0|    154050|    94997|        7765|                High|\n",
            "|      X5|2013|         Asia|  Blue| Electric|      Manual|          2.6|    110142|   114844|        5561|                 Low|\n",
            "|      M5|2017|North America|   Red|   Diesel|   Automatic|          2.6|     86900|    81934|        9755|                High|\n",
            "|5 Series|2017|North America|Silver| Electric|      Manual|          3.1|     57843|   111326|        4880|                 Low|\n",
            "|      X5|2012|       Africa|  Blue| Electric|      Manual|          3.8|    142243|    82677|        7104|                High|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "only showing top 20 rows\n",
            "root\n",
            " |-- Model: string (nullable = true)\n",
            " |-- Year: integer (nullable = true)\n",
            " |-- Region: string (nullable = true)\n",
            " |-- Color: string (nullable = true)\n",
            " |-- Fuel_Type: string (nullable = true)\n",
            " |-- Transmission: string (nullable = true)\n",
            " |-- Engine_Size_L: double (nullable = true)\n",
            " |-- Mileage_KM: integer (nullable = true)\n",
            " |-- Price_USD: integer (nullable = true)\n",
            " |-- Sales_Volume: integer (nullable = true)\n",
            " |-- Sales_Classification: string (nullable = true)\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "df_clean = df.dropna()\n",
        "df_filled = df.fillna(0)"
      ],
      "metadata": {
        "id": "xq0uSKsCuOym"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "from pyspark.sql.functions import col\n",
        "# Example fix: Replace \"column_name\" with an actual column from your DataFrame, e.g., \"Price_USD\"\n",
        "df = df.withColumn(\"Price_USD\", col(\"Price_USD\").cast(\"integer\"))"
      ],
      "metadata": {
        "id": "7gggmf6WuW9S"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "df = df.filter(df[\"Sales_Volume\"] > 2000) # Example: filtering by 'Sales_Volume' being greater than 2000\n",
        "df.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "TJpId5QnudmO",
        "outputId": "7e88fcd2-8d37-41ad-9d58-97273b6a6134"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|   Model|Year|       Region| Color|Fuel_Type|Transmission|Engine_Size_L|Mileage_KM|Price_USD|Sales_Volume|Sales_Classification|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|5 Series|2016|         Asia|   Red|   Petrol|      Manual|          3.5|    151748|    98740|        8300|                High|\n",
            "|      i8|2013|North America|   Red|   Hybrid|   Automatic|          1.6|    121671|    79219|        3428|                 Low|\n",
            "|5 Series|2022|North America|  Blue|   Petrol|   Automatic|          4.5|     10991|   113265|        6994|                 Low|\n",
            "|      X3|2024|  Middle East|  Blue|   Petrol|   Automatic|          1.7|     27255|    60971|        4047|                 Low|\n",
            "|7 Series|2020|South America| Black|   Diesel|      Manual|          2.1|    122131|    49898|        3080|                 Low|\n",
            "|      i8|2022|       Europe| White|   Diesel|      Manual|          1.8|    196741|    55064|        7949|                High|\n",
            "|      X3|2016|South America| White|   Diesel|   Automatic|          1.7|     48073|   116482|        8944|                High|\n",
            "|      i8|2019|       Europe| White| Electric|      Manual|          3.0|     35700|    96257|        4411|                 Low|\n",
            "|      i8|2016|North America|  Blue|   Hybrid|      Manual|          1.9|    116863|    74539|        8252|                High|\n",
            "|7 Series|2020|North America|Silver|   Diesel|   Automatic|          3.8|     27403|   100015|        8111|                High|\n",
            "|5 Series|2020|       Africa| White| Electric|      Manual|          2.3|    163444|   119486|        4668|                 Low|\n",
            "|      X1|2017|         Asia| Black| Electric|   Automatic|          1.6|    107918|    34078|        7291|                High|\n",
            "|      M3|2014|North America|Silver|   Hybrid|      Manual|          3.0|    154050|    94997|        7765|                High|\n",
            "|      X5|2013|         Asia|  Blue| Electric|      Manual|          2.6|    110142|   114844|        5561|                 Low|\n",
            "|      M5|2017|North America|   Red|   Diesel|   Automatic|          2.6|     86900|    81934|        9755|                High|\n",
            "|5 Series|2017|North America|Silver| Electric|      Manual|          3.1|     57843|   111326|        4880|                 Low|\n",
            "|      X5|2012|       Africa|  Blue| Electric|      Manual|          3.8|    142243|    82677|        7104|                High|\n",
            "|      X3|2015|North America|   Red|   Diesel|   Automatic|          3.7|    123910|    88091|        8635|                High|\n",
            "|      M5|2011|       Europe|  Grey| Electric|   Automatic|          3.3|     78042|    49507|        9383|                High|\n",
            "|7 Series|2017|North America| Black|   Petrol|   Automatic|          3.2|     58288|   101791|        7521|                High|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "only showing top 20 rows\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "df.printSchema()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "G4x07Vprumet",
        "outputId": "7105538a-e829-4ef0-ca68-b274a09ecff7"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "root\n",
            " |-- Model: string (nullable = true)\n",
            " |-- Year: integer (nullable = true)\n",
            " |-- Region: string (nullable = true)\n",
            " |-- Color: string (nullable = true)\n",
            " |-- Fuel_Type: string (nullable = true)\n",
            " |-- Transmission: string (nullable = true)\n",
            " |-- Engine_Size_L: double (nullable = true)\n",
            " |-- Mileage_KM: integer (nullable = true)\n",
            " |-- Price_USD: integer (nullable = true)\n",
            " |-- Sales_Volume: integer (nullable = true)\n",
            " |-- Sales_Classification: string (nullable = true)\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "print(\"Total records:\", df.count())\n"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "NspFDkIquptR",
        "outputId": "87313ecc-606b-42a4-b7d4-a7c42d25c200"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "Total records: 50000\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "df.describe().show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "V-KwwWB1usxV",
        "outputId": "53b72184-c0cf-4068-ffe2-5b3d8bfc9034"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+-------+--------+-----------------+-------------+-----+---------+------------+------------------+------------------+------------------+------------------+--------------------+\n",
            "|summary|   Model|             Year|       Region|Color|Fuel_Type|Transmission|     Engine_Size_L|        Mileage_KM|         Price_USD|      Sales_Volume|Sales_Classification|\n",
            "+-------+--------+-----------------+-------------+-----+---------+------------+------------------+------------------+------------------+------------------+--------------------+\n",
            "|  count|   50000|            50000|        50000|50000|    50000|       50000|             50000|             50000|             50000|             50000|               50000|\n",
            "|   mean|    NULL|        2017.0157|         NULL| NULL|     NULL|        NULL| 3.247179999999999|      100307.20314|        75034.6009|        5067.51468|                NULL|\n",
            "| stddev|    NULL|4.324459218093149|         NULL| NULL|     NULL|        NULL|1.0090783975411621|57941.509343524616|25998.248881722793|2856.7671252296077|                NULL|\n",
            "|    min|3 Series|             2010|       Africa|Black|   Diesel|   Automatic|               1.5|                 3|             30000|               100|                High|\n",
            "|    max|      i8|             2024|South America|White|   Petrol|      Manual|               5.0|            199996|            119998|              9999|                 Low|\n",
            "+-------+--------+-----------------+-------------+-----+---------+------------+------------------+------------------+------------------+------------------+--------------------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "df.filter(df.Region == \"South America\").orderBy(F.desc(\"Sales_Volume\")).show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "m6iyZaZ4vmpZ",
        "outputId": "b3f8112d-8537-4fcf-a5b2-64b960d6dd23"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|   Model|Year|       Region| Color|Fuel_Type|Transmission|Engine_Size_L|Mileage_KM|Price_USD|Sales_Volume|Sales_Classification|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|5 Series|2024|South America| White| Electric|      Manual|          2.5|     52062|    88572|        9999|                High|\n",
            "|      M5|2019|South America|  Grey|   Petrol|      Manual|          3.8|    141592|   105893|        9998|                High|\n",
            "|      M3|2010|South America|  Blue|   Petrol|   Automatic|          4.3|    191728|    54325|        9996|                High|\n",
            "|      i3|2018|South America|   Red|   Diesel|      Manual|          1.8|    188763|    46568|        9996|                High|\n",
            "|      i8|2013|South America| Black|   Hybrid|      Manual|          2.1|     96520|    48851|        9995|                High|\n",
            "|3 Series|2017|South America|  Grey|   Hybrid|   Automatic|          3.5|     27670|   116299|        9995|                High|\n",
            "|      M5|2022|South America|Silver|   Diesel|   Automatic|          4.2|    154272|   114766|        9994|                High|\n",
            "|      i3|2023|South America|  Blue| Electric|   Automatic|          3.9|     58541|   113784|        9994|                High|\n",
            "|      X6|2018|South America|Silver|   Diesel|      Manual|          3.0|       470|    84546|        9991|                High|\n",
            "|3 Series|2012|South America| White|   Petrol|   Automatic|          2.5|     84277|   111569|        9990|                High|\n",
            "|      X3|2021|South America|   Red|   Petrol|      Manual|          4.8|      2510|    86699|        9989|                High|\n",
            "|      M3|2017|South America| Black|   Hybrid|      Manual|          3.5|     36536|   111905|        9988|                High|\n",
            "|5 Series|2010|South America|   Red| Electric|      Manual|          4.6|     90476|    75309|        9988|                High|\n",
            "|5 Series|2022|South America|  Blue| Electric|   Automatic|          2.1|    149327|    54276|        9988|                High|\n",
            "|3 Series|2017|South America|  Grey|   Diesel|   Automatic|          2.8|     67842|    63859|        9986|                High|\n",
            "|      i8|2010|South America|  Grey|   Hybrid|      Manual|          3.7|    104762|    38845|        9986|                High|\n",
            "|      i8|2019|South America|  Blue|   Diesel|      Manual|          4.0|    123311|   103261|        9985|                High|\n",
            "|7 Series|2023|South America|  Grey|   Hybrid|   Automatic|          4.5|    173701|    63591|        9984|                High|\n",
            "|      X3|2014|South America|  Grey|   Hybrid|   Automatic|          2.3|     62430|    91468|        9982|                High|\n",
            "|      M3|2019|South America|   Red| Electric|   Automatic|          4.5|    172503|    60313|        9981|                High|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "only showing top 20 rows\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "df.createOrReplaceTempView(\"bmw_data\")"
      ],
      "metadata": {
        "id": "uXEQqRIBwG-r"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "sales = spark.read.csv(\"/content/bmw_data.csv\")\n",
        "\n",
        "sales.printSchema()\n",
        "sales.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "lCbjrR3avHRw",
        "outputId": "e64bdea2-ed1a-4a37-c49b-3418831afe01"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "root\n",
            " |-- _c0: string (nullable = true)\n",
            " |-- _c1: string (nullable = true)\n",
            " |-- _c2: string (nullable = true)\n",
            " |-- _c3: string (nullable = true)\n",
            " |-- _c4: string (nullable = true)\n",
            " |-- _c5: string (nullable = true)\n",
            " |-- _c6: string (nullable = true)\n",
            " |-- _c7: string (nullable = true)\n",
            " |-- _c8: string (nullable = true)\n",
            " |-- _c9: string (nullable = true)\n",
            " |-- _c10: string (nullable = true)\n",
            "\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|     _c0| _c1|          _c2|   _c3|      _c4|         _c5|          _c6|       _c7|      _c8|         _c9|                _c10|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|   Model|Year|       Region| Color|Fuel_Type|Transmission|Engine_Size_L|Mileage_KM|Price_USD|Sales_Volume|Sales_Classification|\n",
            "|5 Series|2016|         Asia|   Red|   Petrol|      Manual|          3.5|    151748|    98740|        8300|                High|\n",
            "|      i8|2013|North America|   Red|   Hybrid|   Automatic|          1.6|    121671|    79219|        3428|                 Low|\n",
            "|5 Series|2022|North America|  Blue|   Petrol|   Automatic|          4.5|     10991|   113265|        6994|                 Low|\n",
            "|      X3|2024|  Middle East|  Blue|   Petrol|   Automatic|          1.7|     27255|    60971|        4047|                 Low|\n",
            "|7 Series|2020|South America| Black|   Diesel|      Manual|          2.1|    122131|    49898|        3080|                 Low|\n",
            "|5 Series|2017|  Middle East|Silver|   Diesel|      Manual|          1.9|    171362|    42926|        1232|                 Low|\n",
            "|      i8|2022|       Europe| White|   Diesel|      Manual|          1.8|    196741|    55064|        7949|                High|\n",
            "|      M5|2014|         Asia| Black|   Diesel|   Automatic|          1.6|    121156|   102778|         632|                 Low|\n",
            "|      X3|2016|South America| White|   Diesel|   Automatic|          1.7|     48073|   116482|        8944|                High|\n",
            "|      i8|2019|       Europe| White| Electric|      Manual|          3.0|     35700|    96257|        4411|                 Low|\n",
            "|3 Series|2012|  Middle East| White| Electric|      Manual|          2.4|    101595|   117995|         500|                 Low|\n",
            "|      i8|2016|North America|  Blue|   Hybrid|      Manual|          1.9|    116863|    74539|        8252|                High|\n",
            "|7 Series|2020|North America|Silver|   Diesel|   Automatic|          3.8|     27403|   100015|        8111|                High|\n",
            "|5 Series|2020|       Africa| White| Electric|      Manual|          2.3|    163444|   119486|        4668|                 Low|\n",
            "|      X1|2017|         Asia| Black| Electric|   Automatic|          1.6|    107918|    34078|        7291|                High|\n",
            "|      M3|2014|North America|Silver|   Hybrid|      Manual|          3.0|    154050|    94997|        7765|                High|\n",
            "|      X5|2013|         Asia|  Blue| Electric|      Manual|          2.6|    110142|   114844|        5561|                 Low|\n",
            "|      M5|2017|North America|   Red|   Diesel|   Automatic|          2.6|     86900|    81934|        9755|                High|\n",
            "|5 Series|2017|North America|Silver| Electric|      Manual|          3.1|     57843|   111326|        4880|                 Low|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "only showing top 20 rows\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "sales.createOrReplaceGlobalTempView(\"car_sales\")\n",
        "df.write.mode(\"overwrite\").saveAsTable(\"car_sales\") #Save as table and overwrite table if exits"
      ],
      "metadata": {
        "id": "MrsEARUVy4S6"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "df2 = spark.sql(\"SELECT * FROM car_sales\")\n",
        "df2.printSchema()\n",
        "df2.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "uhHFTrX0zIw2",
        "outputId": "049f8221-499e-4dba-d9e4-a29086c0ab10"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "root\n",
            " |-- Model: string (nullable = true)\n",
            " |-- Year: integer (nullable = true)\n",
            " |-- Region: string (nullable = true)\n",
            " |-- Color: string (nullable = true)\n",
            " |-- Fuel_Type: string (nullable = true)\n",
            " |-- Transmission: string (nullable = true)\n",
            " |-- Engine_Size_L: double (nullable = true)\n",
            " |-- Mileage_KM: integer (nullable = true)\n",
            " |-- Price_USD: integer (nullable = true)\n",
            " |-- Sales_Volume: integer (nullable = true)\n",
            " |-- Sales_Classification: string (nullable = true)\n",
            "\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|   Model|Year|       Region| Color|Fuel_Type|Transmission|Engine_Size_L|Mileage_KM|Price_USD|Sales_Volume|Sales_Classification|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "|5 Series|2016|         Asia|   Red|   Petrol|      Manual|          3.5|    151748|    98740|        8300|                High|\n",
            "|      i8|2013|North America|   Red|   Hybrid|   Automatic|          1.6|    121671|    79219|        3428|                 Low|\n",
            "|5 Series|2022|North America|  Blue|   Petrol|   Automatic|          4.5|     10991|   113265|        6994|                 Low|\n",
            "|      X3|2024|  Middle East|  Blue|   Petrol|   Automatic|          1.7|     27255|    60971|        4047|                 Low|\n",
            "|7 Series|2020|South America| Black|   Diesel|      Manual|          2.1|    122131|    49898|        3080|                 Low|\n",
            "|      i8|2022|       Europe| White|   Diesel|      Manual|          1.8|    196741|    55064|        7949|                High|\n",
            "|      X3|2016|South America| White|   Diesel|   Automatic|          1.7|     48073|   116482|        8944|                High|\n",
            "|      i8|2019|       Europe| White| Electric|      Manual|          3.0|     35700|    96257|        4411|                 Low|\n",
            "|      i8|2016|North America|  Blue|   Hybrid|      Manual|          1.9|    116863|    74539|        8252|                High|\n",
            "|7 Series|2020|North America|Silver|   Diesel|   Automatic|          3.8|     27403|   100015|        8111|                High|\n",
            "|5 Series|2020|       Africa| White| Electric|      Manual|          2.3|    163444|   119486|        4668|                 Low|\n",
            "|      X1|2017|         Asia| Black| Electric|   Automatic|          1.6|    107918|    34078|        7291|                High|\n",
            "|      M3|2014|North America|Silver|   Hybrid|      Manual|          3.0|    154050|    94997|        7765|                High|\n",
            "|      X5|2013|         Asia|  Blue| Electric|      Manual|          2.6|    110142|   114844|        5561|                 Low|\n",
            "|      M5|2017|North America|   Red|   Diesel|   Automatic|          2.6|     86900|    81934|        9755|                High|\n",
            "|5 Series|2017|North America|Silver| Electric|      Manual|          3.1|     57843|   111326|        4880|                 Low|\n",
            "|      X5|2012|       Africa|  Blue| Electric|      Manual|          3.8|    142243|    82677|        7104|                High|\n",
            "|      X3|2015|North America|   Red|   Diesel|   Automatic|          3.7|    123910|    88091|        8635|                High|\n",
            "|      M5|2011|       Europe|  Grey| Electric|   Automatic|          3.3|     78042|    49507|        9383|                High|\n",
            "|7 Series|2017|North America| Black|   Petrol|   Automatic|          3.2|     58288|   101791|        7521|                High|\n",
            "+--------+----+-------------+------+---------+------------+-------------+----------+---------+------------+--------------------+\n",
            "only showing top 20 rows\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "df3 = spark.sql(\"SELECT Year, SUM(Price_USD) AS total_023_sales FROM car_sales GROUP BY year ORDER BY SUM(Price_USD) DESC LIMIT 5\")\n",
        "df3.printSchema()\n",
        "df3.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "collapsed": true,
        "id": "AeMOSxNm0FQd",
        "outputId": "84b30f4b-2140-4e4f-c126-e5bb1e952c85"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "root\n",
            " |-- Year: integer (nullable = true)\n",
            " |-- total_023_sales: long (nullable = true)\n",
            "\n",
            "+----+---------------+\n",
            "|Year|total_023_sales|\n",
            "+----+---------------+\n",
            "|2022|      213012562|\n",
            "|2024|      209595431|\n",
            "|2019|      206642969|\n",
            "|2016|      206326338|\n",
            "|2021|      205447353|\n",
            "+----+---------------+\n",
            "\n"
          ]
        }
      ]
    }
  ]
}